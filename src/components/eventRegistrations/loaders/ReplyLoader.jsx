...
function ReplyLoader({ replyingStatus, error }) {
  const textPrompts = {
    replying: "Replying to the event registration...",
    waiting: "Please wait...",
    success: "Event Registration Reply Sent!",
    error: "Error replying to event registration. Please try again.",
  };

  const [loadingText, setLoadingText] = useState(
    textPrompts[replyingStatus] || "Replying to the event registration..."
  );
  const [loading, setLoading] = useState(replyingStatus === "replying");

  useEffect(() => {
    if (["success", "error"].includes(replyingStatus)) {
